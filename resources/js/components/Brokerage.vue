              ...
              <v-container>
                <v-row>
                  ...
                  <v-col cols="12" sm="6" md="6">
                    <v-text-field
                      v-model="
                                                editedItem.broker.broker_license
                                            "
                  ...
                  <v-col cols="12" sm="6" md="6">
                    <v-checkbox v-model="editedItem.broker.lock_color" label="Lock logo and color"></v-checkbox>
                  </v-col>
                  <v-col cols="12" sm="6" md="6">
                  ...
                  <v-col cols="12" sm="12" md="6">
                    Banner Color
                    <v-color-picker v-model="editedItem.broker.banner_color" flat></v-color-picker>
                  </v-col>
                  <v-col cols="12" sm="12" md="6">
                    Body Color
                    <v-color-picker v-model="editedItem.broker.body_color" flat></v-color-picker>
                  </v-col>
                  <v-col cols="12" sm="12" md="6">
                    Button Color
                    <v-color-picker v-model="editedItem.broker.button_color" flat></v-color-picker>
                  </v-col>
                </v-row>
              </v-container>
            </v-card-text>
...
export default {
  ...
  methods: {
    ...
    selectFile(e) {
      let reader = new FileReader();
      reader.onload = (eV) => {
        this.editedItem.broker.logo = eV.target.result;
      };
      reader.readAsDataURL(e);
    },
    initialize() {
    ...
    save() {
      axios
        ...
        .then((response) => {
          Toast.fire({
            icon: "success",
            title: "Brokarage Created Successfully",
          });
          this.initialize();
          this.close();
        })
    ...
    update() {
      axios
        .put("/api/broker/" + this.editedItem.id, {
          ...this.editedItem,
        })
        .then((response) => {
          Toast.fire({
